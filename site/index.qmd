---
pagetitle: "HPC Spring School"
page-layout: full
search: true
image: img/parallel-trains.jpg
image-alt: "Data center"
repo-actions: [edit, issue]

about:
  template: solana
  image-width: 30em
  id: hero-heading


listing:
  - id: programme
    contents: 
    - /programme/day-*.qmd
    type: table
    sort: 
    - "date asc"
    - "title asc"
    fields: [image, date, subtitle, title, author]
    field-display-names:
      date: "Date"
      subtitle: "Time"
      title: "Workshop"
      author: "Trainer(s)"
    date-format: "MMMM DD, YYYY"
    sort-ui: false
    filter-ui: false

  - id: support
    contents:
    - path: https://eurocc-netherlands.nl/
      image: img/eurocc.png
      title: "**EuroCC**"
      description: "EuroCC aims to build a European network of 33 national HPC competence centres to bridge the existing HPC skills gaps while promoting cooperation across Europe. The establishment of a European network of 33 national HPC competence centres (NCCs) is coordinated by the EuroCC project. This falls under The European High Performance Computing Joint Undertaking (EuroHPC JU) from the European Union. "
    - path: https://surf.nl
      image: img/surf_logo/SURF%20fc%20logo's/SURF_fc.svg
      title: "**SURF**"
      description: "SURF is a cooperative association of Dutch educational and research institutions in which the members combine their strengths. Within SURF, we work together to acquire or develop the best possible digital services, and to encourage knowledge sharing through continuous innovation. The members are the owners of SURF."
    - path: https://www.uu.nl/en/research/digital-competence-center
      image: https://www.uu.nl/sites/default/files/cm_hs_avatar_corporate.png
      title: "**UDCC**"
      description: "The UDCC (Utrecht Digital Competence Center) is the network of all support teams at Utrecht University for help with research data, software, and infrastructure. We exist to traverse borders, increase visibility, and experience a sense of community in co-creating digital competencies to support research at Utrecht University. "
    type: grid
    categories: false
    max-items: 3
    max-description-length: 500
    image-height: 300px
    feed: false
---



::: {#hero-heading}
## HPC Spring School @ Utrecht University

Are you ready to boost your skills in scientific computation, parallel programming, and High Performance Computing? Then please take part in the HPC Spring School at Utrecht University!

During this Spring School, experts from the UU and SURF offer a wide range of sessions spread over ten days.

The HPC Spring School will take place from 1 June till 12 June at Utrecht University; all sessions are in-person events. 

:::

<br><br>

## Pre-registration Open

The program will be announced and the registration will be open within a week from now. Pre-register using the box below, you will then receive an email notification when the programme is published and registration opens!

<br><br>

<iframe src="https://survey.uu.nl/jfe/form/SV_bEMHTdP03YlV32C" height="1000px" width="1000px"></iframe>


Registration is open for all researchers in the Netherlands. There is a limited number of spots for thesis students. You can design your own program and register for one or multiple workshops of your interest. Note that some workshops have prerequisites, so please make sure to check the requirements before registering.

## Programme (UNDER CONSTRUCTION, expected live date 11 march)

::: {#programme}
:::

## Supported by
:::{#support}
:::



